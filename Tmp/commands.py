...
async def search_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПОЛУЧАЕМ ФИЛЬТРЫ ИЗ НОВОЙ СИСТЕМЫ
    from src.storage.repositories.filter_repo import filter_repo

    filters_text = "python junior"  # Значение по умолчанию

    async for session in db.get_session():
        # Получаем структурированные фильтры
        structured_filters = await filter_repo.get_all_filters(session, user_id)

        if structured_filters:
            # Преобразуем в строку для API HH
            filters_text = build_hh_query(structured_filters)
            logger.info(f"Использую фильтры из новой системы: {filters_text}")
        else:
            # Пробуем получить из старой системы (для обратной совместимости)
            user = await user_repo.get_user(session, user_id)
            if user and user.search_filters:
                filters_text = user.search_filters
                logger.info(f"Использую фильтры из старой системы: {filters_text}")

    logger.info(f"Поиск вакансий по фильтрам: {filters_text}")

    # Ищем вакансии
    try:
        vacancies = await hh_client.search_vacancies(filters, per_page=5)
        ...
        # Отправляем первую вакансию с inline-кнопками
...
def build_hh_query(structured_filters: dict) -> str:
    ...
    -> "Python middle Москва Django PostgreSQL"
    """
    ...
    # Опыт (переводим на русский для лучшего поиска)
    experience_map = {
    ...
    # Город/регион (кроме специальных значений)
    ...
    # Если ничего не нашлось, возвращаем дефолт
    if not parts:
        return "python junior"
...
async def send_vacancy_with_buttons(update, context, index: int):
    ...
    try:
        ...
        # Создаем inline-клавиатуру для этой вакансии
